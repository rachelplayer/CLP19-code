/* 
    ...
    This file was run to obtain HElib invariant noise budget data in Table 5
*/
...
NTL::xdouble get_invariant_noise_budget(Ctxt encrypted, FHESecKey secret_key, long p);
...
NTL::xdouble get_invariant_noise_budget(Ctxt encrypted, FHESecKey secret_key, long p)
{
    NTL::xdouble noise, log_noise,log_max, invariant_noise_budget;
    NTL::ZZ q, max; 
    noise = get_noise(encrypted, secret_key);
    log_noise = log(noise)/log(NTL::xdouble(2));
    q = encrypted.getContext().productOfPrimes(encrypted.getPrimeSet());
    max = q/2 - p;
    log_max = log(max)/log(2);
    invariant_noise_budget = log_max - log_noise;
    return invariant_noise_budget;
}
...
int main()
{
    ...
    while (true)
    {
        cout << "\n HElib invariant noise budget experiments:" << endl << endl;
...
void test_noise(int trials)
{
    ...
    for (int i = 0; i < trials; i++)
    {
        ...
        auto fresh_noise = get_invariant_noise_budget(encrypted1, secret_key, p);
        ...
        auto add_noise = get_invariant_noise_budget(encrypted1, secret_key, p);
        ...
        auto mult_noise = get_invariant_noise_budget(encrypted3, secret_key, p);
        ...
        auto mod_switch_noise = get_invariant_noise_budget(encrypted3, secret_key, p);
    ...
    cout << "Standard deviation of invariant noise budget observed: " << std_dev_fresh << endl;     
    cout << endl;

    cout << "Addition c3 = (c1+c2):" << endl;
    cout << "Mean noise budget observed: " << mean_add_observed << endl;        
    cout << "Standard deviation of invariant noise budget observed: " << std_dev_add << endl;   
    cout << endl; 

    cout << "Multiplication c4 = c3 * c2 = (c1+ c2) * c2: " << endl;  
    cout << "Mean noise budget observed: " << mean_mult_observed << endl;
    cout << "Standard deviation of invariant noise budget observed: " << std_dev_mult << endl;    
    cout << endl;   
    ...
    if(is_not_2048)
    {
        ...
        cout << "Standard deviation of invariant noise budget observed: " << std_dev_mod_switch << endl;    
    cout << endl;
    }
