...
namespace seal
{
    ...
    int Decryptor::new_noise_budget(const Ciphertext &encrypted)
    {
        /*
        Everything until marked is the same as in Decryptor::invariant_noise_budget()
        */
        ...
        Currently, noise_poly contains q * v_invariant.
        Note that 
            (t/q) * ct(s) = (t/q) * Delta * m + v + at = m + v - (r_t(q)/q) * m + at
        and (t/q) * ct(s) = m + v_invariant + at
        Hence v_invariant = v - (r_t(q)/q) * m, 
        So, q * v = q * v_invariant + r_t(q) * m
        */

        // Decrypt the ciphertext to obtain m mod t
        Plaintext message_poly(coeff_count);
        decrypt(encrypted, message_poly);

        
        // Calculate r_t(q) * m
        // Inspired by Encryptor::preencrypt()

        auto r_t_q = context_data.upper_half_increment();
        ...
        auto plain_coeff_count = message_poly.coeff_count();
        ...
        // Multiply plain by scalar r_t_q and reposition if in upper-half.
        for (size_t i = 0; i < plain_coeff_count; i++)
        {
            if (message_poly[i] >= plain_upper_half_threshold)   
            {
                // Loop over primes
                for (size_t j = 0; j < coeff_mod_count; j++)
                {
					uint64_t adjusted_coeff = message_poly[i] + plain_upper_half_increment[j];
                    unsigned long long temp[2]{ 0, 0 };
                    multiply_uint64(r_t_q[j], adjusted_coeff, temp);
                    adjusted_coeff = barrett_reduce_128(temp, coeff_modulus[j]);
                    noise_poly[i + (j * coeff_count)] = add_uint_uint_mod(
                        noise_poly[i + (j * coeff_count)], adjusted_coeff, coeff_modulus[j]);      
                }
            }
            else
            {
                ...
                {
                    unsigned long long temp[2]{ 0, 0 };
                    multiply_uint64(r_t_q[j], message_poly[i], temp);
                    uint64_t adjusted_coeff = barrett_reduce_128(temp, coeff_modulus[j]);
                    noise_poly[i + (j * coeff_count)] = add_uint_uint_mod(
                        noise_poly[i + (j * coeff_count)], adjusted_coeff, coeff_modulus[j]);      
                }
            }
        ...
        // Compute the infinity norm of noise_poly (i.e. || q * v ||, for v the new noise)
        poly_infty_norm_coeffmod(noise_poly.get(), coeff_count, coeff_mod_count,
            context_data.total_coeff_modulus(), destination.get(), pool_);

        // Compute log(|| q * v ||)
        int log_q_times_v = get_significant_bit_count_uint(destination.get(), coeff_mod_count);

        // Approximate log((q/2) - t^2) as log(q/2)
        int noise_budget_limit = context_data.total_coeff_modulus_bit_count() - 1;

        // The noise budget is log( (q/2) - t^2) - log( q * || v || )
        int bit_count_diff = noise_budget_limit - log_q_times_v;
